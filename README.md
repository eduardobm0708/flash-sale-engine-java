...
Este sistema simula o backend de um e-commerce lidando com milhares de requisições por segundo para um item de estoque limitado. O objetivo principal é resolver o problema de **Condição de Corrida** em sistemas distribuídos.

### 🏆 Desafios Técnicos Resolvidos

* **Prevenção Total de Overselling:** Utilização de **Redis + Lua Script** para garantir operações atômicas de decremento de estoque. Isso elimina a concorrência desleal entre threads que ocorre em bancos de dados tradicionais.
...
Siga os passos abaixo para iniciar a aplicação e testar o sistema de Flash Sale:
...
2. Iniciar a aplicação Spring Boot via sua IDE favorita (IntelliJ, Eclipse) ou usando Maven:
    ```bash
    mvn spring-boot:run
    ```
